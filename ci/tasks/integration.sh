...
scp -i ssh.key -o "StrictHostKeyChecking no" -r voyager-ipam-service  ${INTEGRATION_VM_USER}@${INTEGRATION_VM_IP}:$project_path

integration_file=$PWD/voyager-ipam-service/ci/tasks/run-integration.sh
